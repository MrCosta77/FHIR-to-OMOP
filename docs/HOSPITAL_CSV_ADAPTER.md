...
engine. It does not retrieve concepts, call an LLM, write DuckDB or publish a
mapping.
...
clinical_mapping_core SELECT/ABSTAIN contract
      |
      v
existing blinded review + adjudication + STCM gate
```
...
This milestone stops before an end-to-end CSV ingestion pipeline. The next step
must connect each validated record to the existing domain-specific Athena
retrieval and proposal persistence without creating a second publication path.
Only adjudicated mappings may enter STCM or the OMOP CDM.
